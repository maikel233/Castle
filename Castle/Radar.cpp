...
Vector2D WorldToRadar(const Vector location, const Vector origin, const Vector angles, int width, float scale = 16.f)
{
...
ImColor Radar::GetRadarPlayerColor(C_BasePlayer* player, bool visible)
{
	...
	if (Settings::Radar::teamColorType == TeamColorType::RELATIVE_)
	{
	...
	else if (Settings::Radar::teamColorType == TeamColorType::ABSOLUTE_)
	{
...
void Radar::RenderWindow()
{
	...
	if (ImGui::Begin("Radar", &Settings::Radar::enabled, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_ShowBorders | ImGuiWindowFlags_NoTitleBar))
	{
		...
		Vector localplayer_angles;
		pEngine->GetViewAngles(localplayer_angles);
		...
		for (int i = 1; i < pEntityList->GetHighestEntityIndex(); i++)
		{
			...
			if (classId == EClassIds::CCSPlayer)
			{
				...
				C_BasePlayer* observer_target = (C_BasePlayer*)pEntityList->GetClientEntityFromHandle(localplayer->GetObserverTarget());
